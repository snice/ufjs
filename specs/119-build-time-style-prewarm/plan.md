...
### 3.5 实现中的调整（2026-09-24）

1. **快照前置而不是追加**：单包 bundle 在执行过程中就同步挂载了首页（`main.ts` 调
   `createFjsApp().mount()`），追加在末尾的快照来不及生效。改为写在文件第一行；有外部
   sourcemap 时在 `mappings` 前补一个 `;`（整体下移一行）。
2. **PrimJS 的 `JSON.parse` 会把含 `\u0000` 的字符串截成空串**（Node 正常，fjsrun 实测）。
   `font` 简写的待展开标记 `FONT_PENDING` 原来是 `\0font\0`，vant 图标的字体声明经快照
   还原后变成空值、少了字形伪元素。标记改为 `\u0006font\u0006`（引擎内部值，不过桥）；
   CLI 抓取时若快照 JSON 仍含 `\u0000` 则该页不带快照并告警。
3. **对拍口径**：逐节点解析后的样式与结构相同，原始字节允许不同——快照按内容去重
   对象，DefineStyle 更少（vant-form 冷态帧 78 KB → 59 KB）。spec §6.3 已改。
4. **继承的变量表存引用**（`CUSTOM_INHERITED`）：计算节点的变量表多数就是父节点
   （或 `:root`）那张 ~500 个 token 的表，原样存让 `/about` 这种小页的快照也有 32 KB；
   存引用后 8 KB，vant-form 99 → 75 KB。
