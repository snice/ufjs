...
没写 `group` 的页面不进目录。
页面文件按分组放目录：`pages/basic/`、`pages/interaction/`、`pages/vant/`、
`pages/nutui/`，路由即目录路径（`/vant/basic`、`/nutui/button`…，specs/141）。
...
绘制，Web 上内联 SVG，`/basic/icons` 页面用它。demo 对它就是一条 npm 依赖——和别人装它
的方式完全一样。
...
`/interaction/drag` 和 `/interaction/dnd` 两个页面是触摸事件的例子：块拖拽（含多指）和拖拽排序
