...
// FJS_ENGINE_QUICKJS comes from the podspec's pod_target_xcconfig when the
// quickjs flavor is vendored (spec 105; it used to be a header the engine
// runner wrote into this directory, which is shared by every host using the
// package). The two flavors expose different ABI surfaces — quickjs has no
// fjs_vm_debugger_* — so the declarations below must not assume the
// debugger exists.
