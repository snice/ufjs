...
不能。它们基于 DOM 和浏览器 CSS。ufjs 的界面由[内置组件](./components)搭建，外观参照 WeUI。

### 能用哪些 npm 包

纯 JS 逻辑库基本都可以。已经验证过的有 dayjs、mitt、valibot、immer、es-toolkit、pinia、vue-i18n、@vueuse/motion、Anime.js，以及 ECharts、F2、PixiJS、three.js（需要少量平台垫片，见 hello-fjs 的 `src/adapters/`）。不能用的是依赖 DOM（`document`、`window` 上的 DOM API）或 Node 内置模块的库。
